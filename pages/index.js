    ...
    <div className="min-h-screen aura-bg text-[#1e293b] font-outfit selection:bg-[#4f46e5] selection:text-white">
      ...
      <div className="max-w-[1100px] mx-auto px-5 py-8">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-8">
            ...
            {/* CSSPS Placement Card - Restored with Grayscale logic */}
            {/* CSSPS Placement Card - Stays visible but disabled when empty */}
<section className={`glass-card p-8 relative overflow-hidden transition-all duration-500 ${stock.CSSPS <= 0 ? 'opacity-50 grayscale' : ''}`} id="csspsForm">
  {stock.CSSPS <= 0 && (
    <div className="absolute top-4 right-[-35px] bg-red-500 text-white text-[10px] font-bold py-1 px-10 rotate-45 z-10 shadow-md">
      OUT OF STOCK
    </div>
  ...
  {/* The container below becomes unclickable if stock is 0 */}
  <div className={`space-y-4 ${stock.CSSPS <= 0 ? 'pointer-events-none' : ''}`}>
      ...
      {hasMounted && (
          <PaystackButton 
            ...
            currency="GHS"
            disabled={stock.CSSPS <= 0}
            className={`w-full py-4 rounded-[14px] font-bold transition-all ${
              stock.CSSPS <= 0 
              ? 'bg-slate-400 cursor-not-allowed shadow-none' 
              : 'bg-gradient-to-br from-[#4f46e5] to-[#06b6d4] text-white shadow-lg shadow-indigo-200 hover:-translate-y-1'
            }`}
          />
      )}
  </div>
