...
export default function ThankYou() {
  ...
  // Specific portal mapping based on voucher type
  const getPortalDetails = (type) => {
    const t = type?.toUpperCase() || "";
    if (t.includes('BECE')) {
      return { name: 'BECE Portal', url: 'https://eresults.waecgh.org' };
    }
    if (t.includes('WASSCE') || t.includes('NOVDEC')) {
      return { name: 'WASSCE Portal', url: 'https://ghana.waecdirect.org' };
    }
    if (t.includes('CSSPS') || t.includes('PLACEMENT')) {
      return { name: 'Placement Portal', url: 'https://www.cssps.gov.gh/' };
    }
    return null;
  };
  ...
  const downloadPDF = () => {
    ...
    doc.text('Your official voucher receipt is below.', 14, 35);

    const tableColumn = ["Voucher Type", "Serial Number", "PIN"];
    const tableRows = vouchers.map(v => [v.type, v.serial, v.pin]);
    ...
    doc.autoTable({
      ...
      startY: 45,
      theme: 'grid',
      headStyles: { fillStyle: 'solid', fillColor: [79, 70, 229], textColor: 255 },
    });

    const finalY = doc.lastAutoTable.finalY || 45;
    doc.setFontSize(9);
    doc.text('Keep this document safe. Use the PINs on the official WAEC/CSSPS portals.', 14, finalY + 10);
    ...
    <div className="min-h-screen bg-[#FDFDFD] flex flex-col items-center py-8 px-4 sm:justify-center font-outfit">
      ...
      <div className="w-full max-w-2xl bg-white border border-slate-200 rounded-[2.5rem] p-6 md:p-12 shadow-xl shadow-slate-200/50">
        ...
        <div className="text-center mb-8">
            ...
            <p className="text-sm md:text-base text-slate-500 font-medium">Vouchers sent via SMS and displayed below.</p>
        </div>
        ...
        {/* Action Grid: PDF Download & Specific Portal Links */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-8">
          ...
          {/* Dynamic Links specific to purchased vouchers */}
